...
PORTFOLIO.pages = [
  ...
  },
  {
    ...
    blocks: [
      ...
      {
        "t": "sub",
        "html": "군 복무 중 <b>사지방(사이버지식정보방)</b>에서 바이브 코딩으로 웹 기반 게임과 사이트, 앱을 만들고 있습니다. 개발 환경이 제한된 조건에서도 손을 놓지 않기 위해 시작했고, 지금은 <b>아키텍처 설계 → 구현 → 테스트 → 배포</b>까지 혼자 돌리는 작업 방식으로 자리 잡았습니다. 그중 <b>러브 메모리</b>는 구글 플레이에 <b>정식 출시</b>했고, 나머지는 개발 중입니다."
      },
      ...
      {"t":"sub","html":"전부 같은 방식으로, 사지방에서 혼자 만들고 있습니다.","variant":"tight"},
      {
        ...
        "items": [
          ...
          {"n":"WEB · APP","title":"러브 메모리","desc":"연인이 함께 쓰는 캘린더. <b>구글 플레이에 정식 출시했습니다.</b> 다음 쪽에 화면과 설계를 따로 정리했습니다. <a href=\"#\" data-goto=\"love-memory\">자세히 보기</a>"},
  ...
  },
  {
    ...
    blocks: [
      ...
      {
        "t": "sub",
        "html": "군 복무 중 사지방에서 혼자 만든 앱입니다. 기획·디자인·개발·운영을 혼자 하고 있고, 비공개 테스트에서 받은 피드백을 반영해 <b>구글 플레이에 정식 출시</b>했습니다. 지금은 실사용자를 받으며 운영하고 있습니다. 아래는 소개 영상과 실제 동작 화면입니다."
      },
      {"t":"shot","alt":"러브 메모리 소개 영상"},
      {
        "t": "shots",
        "items": [
          {"src":"assets/img/love-memory/calendar-month.jpg","alt":"러브 메모리 월 달력 화면","cap":"월 달력 — 날짜에 그날 사진이 깔리고, 여러 날 이어지는 일정은 띠로 이어집니다"},
          {"src":"assets/img/love-memory/day-sheet.jpg","alt":"러브 메모리 날짜 시트 화면","cap":"날짜 시트 — 그날의 사진, 기념일, 시간별 일정, 할 일이 한 화면에"},
          {"src":"assets/img/love-memory/day-story.jpg","alt":"러브 메모리 이날의 이야기 화면","cap":"이날의 이야기 — 서로 남긴 말과 이모지 반응, 그날의 일정"},
          {"src":"assets/img/love-memory/photo-detail.jpg","alt":"러브 메모리 사진 상세 화면","cap":"사진 상세 — 대표 사진 지정, 반응과 댓글, 위치 추가"},
          {"src":"assets/img/love-memory/settings-theme.jpg","alt":"러브 메모리 테마 설정 화면","cap":"테마 — 프리셋 8종과 다섯 색 직접 지정, 글씨 크기 4단계"}
        ]
      },
      {
        "t": "vibe",
        ...
        "year": "2026",
        "lead": [
          "일정만 공유하는 앱은 이미 많아서, <b>지나간 날을 다시 열어보게 만드는 것</b>을 목표로 잡았습니다. 사진과 그날 주고받은 말이 날짜에 그대로 붙어 있고, 달력을 넘기면 지난 몇 달이 사진으로 훑어집니다.",
          "슬로건은 <b>‘평소엔 조용하고, 돌아볼 때 좋은 앱’</b>입니다. 알림으로 붙잡아 두는 대신, 열었을 때 그날이 온전히 남아 있는 쪽을 택했습니다."
        ],
        ...
        "decs": [
          {"n":"DESIGN 01","title":"둘만의 공유 모델","desc":"모든 기록에 누가 남겼는지가 붙습니다. 사진에는 <b>‘파트너가 올림’</b>, 일정에는 <b>‘파트너가 등록’</b>, 이야기에는 이름이 따라붙고, 할 일은 아예 ‘내 할 일’과 ‘파트너 할 일’로 나눴습니다. 2인이라 권한 규칙 자체는 단순하지만, 누가 무엇을 했는지 보이지 않으면 함께 쓰는 느낌이 사라진다고 봤습니다. 두 사람이 같은 데이터를 보는 부분은 <b>Firebase</b>로 처리했습니다."},
          {"n":"DESIGN 02","title":"하루를 하나의 화면으로","desc":"날짜를 누르면 그날의 사진, 기념일, 일정, 할 일, 주고받은 이야기가 <b>한 시트에 모입니다.</b> 여행 일정은 13:00 부산역 도착부터 23:00 밤 산책까지 시간과 장소가 붙은 타임라인으로 펼쳐집니다. 탭을 옮겨 다니지 않고 하루를 열면 그날이 전부 보이게 했습니다."},
          {"n":"DESIGN 03","title":"여러 날에 걸친 일정","desc":"‘부산 여행’처럼 며칠씩 이어지는 일정은 달력에서 <b>한 줄의 띠로 이어집니다.</b> 주 경계를 넘어가면 주 단위로 잘라 이어 붙여야 하고, 같은 날 일정이 겹치면 줄을 나눠 쌓아야 합니다. 달력에서 손이 가장 많이 간 부분입니다."},
          {"n":"DESIGN 04","title":"사진이 곧 그날의 표지","desc":"날짜 칸 배경에 그날 사진이 깔려서, <b>달력을 넘기는 것만으로 지난 몇 달이 훑어집니다.</b> 사진마다 대표 지정이 가능하고 이모지 반응과 댓글을 달 수 있습니다. 위치를 붙이면 지도 탭에 다녀온 곳으로 모입니다."},
          {"n":"DESIGN 05","title":"색을 직접 고르는 테마","desc":"프리셋 8종(벚꽃·하늘·민트·라벤더·살구·숲·바다·무채색)을 두고, 그걸로 부족하면 <b>배경·카드·포인트·포인트 배경·글자 다섯 색을 직접 지정</b>해 나만의 팔레트로 저장합니다. 화면의 모든 색을 변수 한 겹으로 통과시켜 둔 덕분에 가능한 구조입니다."},
          {"n":"DESIGN 06","title":"글씨 크기 4단계","desc":"작게·보통·크게·더 크게를 설정에 뒀습니다. 오래 들여다보는 앱이고 쓰는 사람마다 편한 크기가 다릅니다. 글씨 크기를 고정값이 아니라 <b>기준 단위로 두고 화면 전체가 함께 커지도록</b> 했습니다."}
        ],
        "foot": "▸ <b>구글 플레이에 정식 출시했습니다.</b> 비공개 테스트에서 받은 피드백을 반영해 출시했고, 지금은 실사용자 피드백을 받으며 업데이트하고 있습니다.<br/>▸ 출시까지의 일정도 이 앱으로 관리했습니다 — 위 달력의 초록색 띠가 그것입니다.",
        "genre": "커플 캘린더"
      }
    ]
  },
